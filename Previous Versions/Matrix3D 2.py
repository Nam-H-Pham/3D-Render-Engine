...
class World:
    ...
    def move_shape(self, shape: Shape3D, axis: str, amount: float) -> None:
    ...
    def draw(self, screen, camera) -> None:
        ...
        for plane in self.floor:
            camera.draw_shape(screen, plane, colour=(100, 100, 100))
...
rect = create_rect_prism(0, -1, 0, 1, 1, 1)
...
while True:
    ...
    if keys[pygame.K_w]:
        w.move_shape(rect, 'z', -movement_amount)
    if keys[pygame.K_s]:
        w.move_shape(rect, 'z', movement_amount)
    if keys[pygame.K_a]:
        w.move_shape(rect, 'x', -movement_amount)
    if keys[pygame.K_d]:
        w.move_shape(rect, 'x', movement_amount)
    if keys[pygame.K_SPACE]:
        w.move_shape(rect, 'y', -movement_amount)
