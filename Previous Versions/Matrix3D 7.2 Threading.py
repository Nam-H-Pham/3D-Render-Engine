...
class Camera:
    ...
    def trace_pixels(self, world, shape: Shape3D) -> None:
        ...
        def render_thread(self, world, shape: Shape3D, image_distance_data, image_data, x_portion_start, x_portion_end) -> None:
            for x in tqdm(range(x_portion_start, x_portion_end), desc="Processing", unit="iteration", total=x_portion_end - x_portion_start):
                for y in range(self.screen.get_height()):
                    ...
                    for tri in shape.tris:
                        ...
                        if lowest_x > x_portion_end or highest_x < x_portion_start:
                            continue
        ...
        for i in range(slices):
            x_portion_start, x_portion_end = x_portions[i]
            thread = threading.Thread(target=render_thread, args=(self, world, shape, image_distance_data, image_data, x_portion_start, x_portion_end))
            thread.start()
...
def create_pyramid(x: float, y: float, z: float, width: float, height: float, depth: float) -> Shape3D:
    points = []
    points.append(Point3D(x, y, z))
    points.append(Point3D(x + width, y, z))
    points.append(Point3D(x + width, y, z + depth))
    ...
    shape = Shape3D(points)
    shape.create_tri(points[0], points[1], points[4])
    shape.create_tri(points[1], points[2], points[4])
    shape.create_tri(points[2], points[3], points[4])
    shape.create_tri(points[3], points[0], points[4])
    shape.create_tri(points[0], points[1], points[2])
    shape.create_tri(points[2], points[3], points[0])
    return shape
        ...
        for line in file:
            ...
            if len(line):
                if line[0] == 'f':
                    ...
                    if len(line) == 4:
                        vertex1 = int(line[2].split('/')[0]) - 1
                        vertex2 = int(line[3].split('/')[0]) - 1
                        vertex3 = int(line[0].split('/')[0]) - 1
                        shape.create_tri(points[vertex1], points[vertex2], points[vertex3])
...
#object = create_pyramid(0.5, 0.5, 0.5, 1, 1, 1)
w.add_shape(object)
