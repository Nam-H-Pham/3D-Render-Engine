...
def create_tri_pyramid(x: float, y: float, z: float, width: float, height: float, depth: float) -> Shape3D:
    points = []
    points.append(Point3D(x, y, z))
    points.append(Point3D(x + width, y, z))
    points.append(Point3D(x + width/2, y + height, z + depth/2))
    points.append(Point3D(x, y, z + depth))
    shape = Shape3D(points)
    shape.connect(points[0], points[1])
    shape.connect(points[1], points[2])
    shape.connect(points[2], points[0])
    shape.connect(points[0], points[3])
    shape.connect(points[2], points[3])
    shape.connect(points[1], points[3])
    return shape
...
shadow = w.create_plane(0.5, 0, 0.5, 1, 1)
shadow.colour = (50, 50, 50)

tri = create_tri_pyramid(5, 0, 5, 0.5, 0.5, 0.5)
tri2 = create_tri_pyramid(5, 1, -5, 0.5, 0.5, 0.5)
tri3 = create_tri_pyramid(5, 2.5, 5, 0.5, 0.5, 0.5)
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_e:
                rect.physics_enabled = not rect.physics_enabled
                print(rect.physics_enabled)

            if event.key == pygame.K_SPACE:
                w.apply_force_axis_aligned(rect, 'y', 0.2)
    ...
    if keys[pygame.K_w]:
        w.apply_force_universal_axis_aligned(rect, 'z', -movement_amount) 
    if keys[pygame.K_s]:
        w.apply_force_universal_axis_aligned(rect, 'z', movement_amount)
    if keys[pygame.K_a]:
        w.apply_force_universal_axis_aligned(rect, 'x', -movement_amount)
    if keys[pygame.K_d]:
        w.apply_force_universal_axis_aligned(rect, 'x', movement_amount)


    tri.rotate_arbitrary(w.world_directions[1].loc, 1)
    tri2.rotate_arbitrary(w.world_directions[1].loc, 1.5)
    tri3.rotate_arbitrary(w.world_directions[2].loc, 2)
    tri3.rotate_arbitrary(w.world_directions[1].loc, 2)

    screen.fill((0, 0, 0))

    flat_velocity = rect.velocity.copy()
    flat_velocity = flat_velocity - w.world_directions[1].loc * np.dot(flat_velocity, w.world_directions[1].loc) / np.dot(w.world_directions[1].loc, w.world_directions[1].loc)
    shadow.move(flat_velocity)
