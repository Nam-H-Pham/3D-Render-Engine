...
class Camera:
    ...
    def trace_pixels(self, world) -> None:
        ...
        name = 'test' + str(len(final_render_image_passes)) + time.strftime("%Y%m%d-%H%M%S") + '.png'
        matplotlib.image.imsave(name, final_render, cmap='gray')
...
main_prism = create_rect_prism(-0.5, 0, -0.5, 1, 1, 1)
main_prism.rotate_arbitrary(np.array([1, 1, 1]), 45)
main_prism.move(np.array([0, 1, 0]))
w.add_shape(main_prism)
w.add_shape(create_rect_prism(-4, 0, -4, 8, 8, 0))
w.add_shape(create_plane(-4, 0, -4, 8, 10))
...
rotation_angle_per_frame = 360 / number_of_rotations
while True:
    ...
    if w.ray_tracing:
        main_prism.rotate_arbitrary(np.array(w.world_directions[1].loc), rotation_angle_per_frame)
