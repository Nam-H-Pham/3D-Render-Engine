...
class Point3D:
    def __init__(self, x, y, z):
        '''Creates a 3D point with the given x, y, and z coordinates'''
        self.loc = np.array([x, y, z])

    def rotate(self, axis: str, angle: float) -> None:
        '''
        Rotates around the given axis by the given angle
    ...
    def rotate_arbitrary(self, axis: np.array, angle: float) -> None:
        '''
        Rotates around the given axis by the given angle
        in relation to a given axis vector

        Used for rotating around the world's y axis
        '''
        angle = np.radians(angle)
    ...
    def move(self, direction_vector: np.array) -> None:
        '''Moves the point by the given direction vector'''
...
class Shape3D(ABC):
    def __init__(self, points: list = [], colour=(255,255,255)) -> None:
        '''Creates a 3D shape with the given points'''
        self.points = points
        self.colour = colour
        self.tris = []
    ...
    def rotate(self, axis: str, angle: float) -> None:
        '''Rotates the shape around the given axis by the given angle'''
    ...
    def move(self, direction_vector: np.array) -> None:
        '''Moves the shape by the given direction vector'''
        for point in self.points:
...
class Physics_Shape3D(Shape3D):
    ...
    def floor_collision(self, world_directions) -> tuple:
        '''
        Returns whether the shape is colliding with the floor

        Returns a tuple of (collision, vertical_vector)
    ...
    def normal_collision(self, world_directions, vertical_vector) -> tuple:
        '''Applies normal collision to the shape'''
...
class World:
    ...
    def add_shape(self, shape: Shape3D) -> None:
        '''Adds the given shape to the world'''
        self.shapes.append(shape)
    
    def rotate(self, axis: str, angle: float) -> None:
        '''Rotates the world around the given axis by the given angle'''
    ...
    def move_shape_axis_aligned(self, shape: Shape3D, axis: str, amount: float) -> None:
        '''Moves the given shape by the given amount along the given axis'''
    ...
    def apply_force_axis_aligned(self, shape: Shape3D, axis: str, amount: float) -> None:
        '''Applies the given force to the given shape along the given axis'''
        direction = self.world_directions[self.camera.get_axis_index(axis)]
        direction = direction.loc
    ...
    def camera_pixel_in_world(self, x: float, y: float) -> Shape3D:
        '''Returns the given pixel in the world'''
    ...
    def create_ray(self, position: np.array, direction: np.array) -> Shape3D:
        '''Creates a ray with the given position and direction'''
        x, y, z = position
        points = []
...
class Camera:
    ...
    def move(self, axis: str, amount: float) -> None:
        '''Moves the camera by the given amount along the given axis'''
        self.loc[self.get_axis_index(axis)] += amount
        self.loc[2] = max(1, self.loc[2])
        
    def get_projection(self, point: Point3D) -> tuple: 
        '''Returns the projection of the given point on the camera'''
    ...
    def draw_all_edges(self, world) -> None:
        '''Draws all the edges of the world'''
    ...
    def draw_all_faces(self, world) -> None:
        '''Draws all the faces of the world'''
    ...
    def trace_pixels(self, world) -> None:
        '''Traces all the pixels of the world for ray tracing'''
        complete_shape = Shape3D()
        for shape in world.shapes:
            for point in shape.points:
                complete_shape.points.append(point)
            ...
            for x in tqdm(range(x_portion_start, x_portion_end), desc="Processing", unit="iteration", total=x_portion_end - x_portion_start, mininterval=0.5):
                for y in range(y_portion_start, y_portion_end):
                    ...
                    if render_pass != 0:
                        ...
                        previous_ray_direction = image_ray_direction_data[x][y]
                    ...
                    for i in range(len(shape.tris)):
                        ...
                        if render_pass == 0:
                            ...
                            prev_x_dir, prev_y_dir = previous_ray_direction[0], previous_ray_direction[1]
                            if prev_x_dir > 0 and x > highest_x:
                                continue
                            elif prev_x_dir < 0 and x < lowest_x:
                                continue

                            if -prev_y_dir > 0 and y > highest_y:
                                continue
                            elif -prev_y_dir < 0 and y < lowest_y:
                                continue
    ...
    def ray_triangle_intersection(self, ray_origin, ray_direction, tri):
        '''Get the intersection point between a ray and a triangle'''
        epsilon = 1e-6
...
def create_plane(x: float, y: float, z: float, width: float, height: float) -> Shape3D:
    '''Creates a plane with the given dimensions'''
    points = []
    points.append(Point3D(x, y, z))
    points.append(Point3D(x + width, y, z))
...
def shape_from_obj(file_name: str) -> Shape3D:
    '''Creates a shape from the given .obj file'''
    points = []
...
if __name__ == "__main__":
    ...
    w.add_shape(shape_from_obj(model_folder + model_name))

    w.add_shape(create_rect_prism(1, 0, 1, 1, 1, 1))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.VIDEORESIZE:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    w.camera.debug = not w.camera.debug
                    if w.camera.debug:
                        w.ray_tracing = True
