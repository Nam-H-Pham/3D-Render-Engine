...
class Shape3D:
    ...
    def apply_physics(self, gravity: float, drag: float, world_directions: list) -> None:
        if self.physics_enabled:
            ...
            self.normal_collision(world_directions)

            movement_vector = self.velocity
            movement_vector = movement_vector * drag

            self.move(movement_vector)
    ...
    def floor_collision(self, world_directions) -> tuple:
        ...
        return (collision, vertical_unit_vector, vertical_vector)
        
    def normal_collision(self, world_directions) -> tuple:
        collision, vertical_unit_vector, vertical_vector = self.floor_collision(world_directions)
        if collision:
            self.velocity = -np.linalg.norm(self.velocity) * vertical_unit_vector * 0.5
...
while True:
    ...
    if keys[pygame.K_w]:
        ...
        w.apply_force_axis_aligned(rect, 'z', -movement_amount) 
    if keys[pygame.K_s]:
        # w.move_shape_axis_aligned(rect, 'z', movement_amount)
        w.apply_force_axis_aligned(rect, 'z', movement_amount)
    if keys[pygame.K_a]:
        # w.move_shape_axis_aligned(rect, 'x', -movement_amount)
        w.apply_force_axis_aligned(rect, 'x', -movement_amount)
    if keys[pygame.K_d]:
        # w.move_shape_axis_aligned(rect, 'x', movement_amount)
        w.apply_force_axis_aligned(rect, 'x', movement_amount)
